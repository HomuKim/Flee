...
public class ProductPaging {
	...
	public ProductPaging(int countItemAll, int pageBlock, int pageSize, int currentPage, String url, String subUrl, String searchUrl) {
		...
		// pageHtml을 작성
		makePageHtml(url, searchUrl, subUrl);
	}
	
	  private void makePageHtml(String url, String searchUrl, String subUrl) {
	      pageHtml.append("<div class=\"paging1\">");
	      pageHtml.append("<ul>");
	      if(startPage>pageBlock) {
	         pageHtml.append("<li><a href=\"" + url + "?page=" + (startPage-pageBlock) + subUrl + searchUrl + "\">&lt;</a></li>");
	      }
	      
	      for(int i=startPage;i<=endPage;i++) {
	         
	         if(i!=currentPage) {
	            pageHtml.append("<li><a href=\"" + url + "?page=" + i + subUrl  + searchUrl + "\">" + i + "</a></li>");            
	         } else {
