...
public class cPaging {
	...
	public cPaging(int countItemAll, int pageBlock, int pageSize, int currentPage, String url, String searchUrl) {
	...
	private void makePageHtml(String url, String searchUrl) {
		...
		if(startPage>pageBlock) {
			pageHtml.append("<li><a href=\"" + url + "&cpage=" + (startPage-pageBlock) + searchUrl + "\">&lt;</a></li>");
		}
		
		for(int i=startPage;i<=endPage;i++) {
			
			if(i!=currentPage) {
				pageHtml.append("<li><a href=\"" + url + "&cpage=" + i  + searchUrl + "\">" + i + "</a></li>");				
			} else {
